...
-- Insert sample users first (password is 'password123' hashed with bcrypt)
...
-- Insert sample reviews (using the IDs from the inserted users and products)
...
-- Insert sample tags (using the review IDs from the inserted reviews)
...
SELECT r.id, u.username, p.name as product, r.rating, LEFT(r.review_text, 50) as review_preview
FROM reviews r
